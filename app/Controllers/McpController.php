...
class McpController extends ResourceController
{
    ...
    private function getSystemPrompt(): string
    {
        ...
        return "تنبيه مهم: آلية العمل ومراحل التنفيذ\n"
             ...
             . "- عند طلب الاستكشاف أو التحليل، استخدم أدوات MCP المتاحة مثل `fetch_new_data` (لجلب البيانات الجديدة حسب التاريخ والدولة والتصنيف افتراضياً ككل)، أو `filter_winning_products` (مع تحديد country='MA' للسوق المغربي)، أو `list_snapshots` / `get_snapshot_by_date` للتواريخ المتاحة، أو `get_saved_products` للاستعلام عن المحفوظات الخاصة بالحساب، أو `save_product` لحفظ وتحديث المنتج في المحفوظات التابعة للحساب.\n"
